...
export default function Compact({ data, coverImage, workData }) {
  return (
    <>
      <section className="cover_image max-w-3xl mx-auto relative">
        <img src={coverImage} alt="cover image" className="w-full min-h-60 max-h-60 object-cover rounded-br-2xl rounded-bl-2xl bg-zinc-300" />
        <Link href="/"><img src="/images/spotlight_logo.png" alt="" className="absolute w-28 top-7" /></Link>
      </section>
      <section className="content max-w-3xl mx-auto px-6 lg:px-0">
        <div className="profile_data max-w-xl mx-auto">
          <div className="col-span-3 text-center">
            <img
              src={`${data?.ProfileImage?.url || '/images/profile_placeholder.png'}`}
              alt="profile image"
              className="w-40 h-40 object-cover rounded-full -mt-20 mb-5 mx-auto border border-gray bg-gray relative z-1"
            />
        ...
        <div className="gallery my-14">
          <WorkGallery theme="compact" data={workData} />
        </div>
      </section>
    </>
  );
